	...
	b.fn.fullpage = function(c) {
		...
		function na(a) {
			...
			if (!R(a.target)) {
				...
				var e = Q(a);
				k || t || (d = S(d), s = d.y, z = d.x, a.find(".fp-slides").length && Math.abs(A - z) > Math.abs(u - s) ? Math.abs(A - z) > b(window).width() / 100 * 5 && (A > z ? h.right && b.fn.fullpage.moveSlideRight() : h.left && b.fn.fullpage.moveSlideLeft()) : c.autoScrolling && !c.scrollBar && Math.abs(u - s) > b(window).height() / 100 * c.touchSensitivity && (u > s ? y("down", e) : s > u && y("up", e)))
			}
		}
		function R(a, d) {
			d = d || 0;
		...
		b.fn.fullpage.setTouchSensitivity = function(a) {
			c.touchSensitivity = a
		};
		b.fn.fullpage.setMouseWheelScrolling = function(a) {
			a ? document.addEventListener ? (document.addEventListener("mousewheel", q, !1), document.addEventListener("wheel", q, !1)) : document.attachEvent("onmousewheel", q) : document.addEventListener ? (document.removeEventListener("mousewheel", q, !1), document.removeEventListener("wheel", q, !1)) : document.detachEvent("onmousewheel", q)
		};
